...
function deleteItem(deleteItemAction){
    ...
    // CRIA UM NOVO ARRAY DE STATUS PENDENTE MENOS O QUE TIVER INDEX SELECIONADO
    document.querySelectorAll('.check').forEach(item => {
        if(item.id !== `checkbox${chave_key}`){
            pendente.push(item.value)
        }
    })
    
    deleteItemAction(produto, quantidade, trocas, valor, valor_unitario, pendente, total)
}

async function deleteItemAction(produto, quantidade, trocas, valor, valor_unitario, pendente, total){
...
async function updateStatus(){
    ...
    window.location.href="<?=$base;?>/expedicao.php?date=<?=$data_emissao;?>"
}

function isCheck(changeChecked){
    ...
    changeChecked(pendente)
}

async function changeChecked(pendente){

    const req = await fetch("<?=$base;?>/changeChecked.php?id=<?=$id;?>",{
        method: 'POST',
        body: JSON.stringify({
            pendente: pendente,
        })
